		...
		, success:function(jsonObj){
			...
			if(dataList.length > 0){
				$.each(dataList, function(index, lec){ //jquery 아닐떄 >> dataList.forEach(function(member)
					...
					let statusText = "";
					if (lec.lol.appStatus == 'STA06') {
				        statusText = '진행중';
				    } else if (lec.lol.appStatus == 'STA07') {
				        statusText = '종강';
				    }
					...
					trTags += `
						<tr data-lec-code="${lec.lecCode }">
                            ...
                            <td style="width: 100px; text-align: center;">
                                <small class="js-lists-values-date text-50">${statusText }</small>
                            </td>
                        </tr>
					`;
				});
...
$(document).on("click","[data-lec-code]", function(event){
	...
	$.ajax({
	 ...
	 window.location.href = cpath + "/professor/cyber/syllabus/syllabusDetail/" + lecCode;
