...
<title>Web Messenger main</title><!--WebSocket messenger main-->
<link type="text/css" rel="stylesheet" href="<%= request.getContextPath() %>/resources/css/table.css"/>
...
// 커넥션 수립
	...
	function random(roomId) {
		<%
			...
			for(int i=0; i<8; i++) {
				rUid += (char)((new Random().nextDouble()*26)+97);//KISA 보안약점 조치
			}
		%>
		return roomId+"."+"<%=rUid%>";
	}
...
<body>
	<form name="usersForm">
		...
		본인 외의 대화상태를 선택하면 대화창이 뜹니다. <br/><!-- 본인 외의 대화상태를 선택하면 대화창이 뜹니다. -->
	...
	<a href="<c:url value="/msg/session.do?username=test"/>">더미 로그인</a>
</body>
</html>
