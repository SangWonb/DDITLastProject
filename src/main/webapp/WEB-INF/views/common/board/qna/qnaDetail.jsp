...
document.getElementById("deleteBtn").addEventListener("click", function() {
    if (confirm("정말로 삭제하시겠습니까?")) {
       ...
       window.location.href = `<c:url value="/common/board/qna/delete/\${cmntCode}"/>`;
    } else {
       alert("삭제가 취소되었습니다.");
    }
 });
...
document.getElementById("modiBtn").addEventListener("click",function(event) {
	var urlParams = new URLSearchParams(window.location.search);
	var cmntCode = urlParams.get('cmntCode');
	window.location.href = `<c:url value="/common/board/qna/updateForm/\${cmntCode}"/>`; //@PV
});
...
function openReplyForm(cmntCode) {
	var url = "${pageContext.request.contextPath}/common/board/qna/qaForm?parentCmntCode=" + cmntCode;
    window.location.href = url;
}
