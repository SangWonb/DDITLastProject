...
<form method="post" id="deleteForm" method="post"
	action="${pageContext.request.contextPath}/professor/cyber/lecturenotice/${la.lecannNo}">
	<input type="hidden" name="lecannNo" id="lecannNo"
		value="${la.lecannNo }"> <input type="hidden" name="lecCode"
		id="lecCode" value="${la.lecCode }">
	<div style="width: 100%; background-color: #FFFFFF;" >
		...
		<div
			...
			<a
				href="${pageContext.request.contextPath}/professor/cyber/lecturenotice/lectureNoticeEdit?lecannNo=${la.lecannNo}"><button
...
<script>
	document
			...
			.addEventListener(
					"click",
					function() {
						window.location.href = `<c:url value="/professor/cyber/${la.lecCode}/lectureNotice"/>`;
					});
	...
	document
			...
			.addEventListener(
					"click",
					function(event) {
						var lecannTitle = "${la.lecannTitle}";
						var lecannContent = "${la.lecannContent}";
						var urlParams = new URLSearchParams(
								window.location.search);
						var lecannNo = urlParams.get('lecannNo');
						window.location.href = `<c:url value="/professor/cyber/lecturenotice/lectureNoticeEdit/\${lecannNo}"/>`;
					});
	...
	document.getElementById("deleteBtn").addEventListener("click", function() {
		if (confirm("정말로 삭제하시겠습니까?")) {
			...
			document.getElementById('lecannNo').value = lecannNo;
